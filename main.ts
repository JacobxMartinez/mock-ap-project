...
function SpawnBonus (num: number) {
    for (let index = 0; index < num; index++) {
        Bonus = sprites.create(img`
            ...
            . . . . . . . . . . . . . . . . 
            `, SpriteKind.Bonus)
        Bonus.setPosition(43, 9)
        Bonus.setBounceOnWall(true)
    }
    Bonus.setVelocity(randint(50, 100), 75)
}
scene.onHitWall(SpriteKind.Player, function (sprite, location) {
    if (sprite.image.equals(img`
        ...
        if (Player1.isHittingTile(CollisionDirection.Right) || Player1.isHittingTile(CollisionDirection.Left)) {
            Player1.vy = 0
        }
    } else {
...
sprites.onOverlap(SpriteKind.Bonus, SpriteKind.Player, function (sprite, otherSprite) {
    sprites.destroy(Bonus, effects.warmRadial, 200)
...
game.onUpdate(function () {
    if (Player1.tileKindAt(TileDirection.Center, assets.tile`myTile0`)) {
        game.gameOver(false)
    }
    if (player_2.tileKindAt(TileDirection.Center, assets.tile`myTile0`)) {
        game.gameOver(false)
    }
    if (Player1.tileKindAt(TileDirection.Center, assets.tile`myTile1`)) {
        game.gameOver(false)
    }
    if (player_2.tileKindAt(TileDirection.Center, assets.tile`myTile1`)) {
        game.gameOver(false)
    }
})
game.onUpdate(function () {
    CameraSprite.setPosition((Player1.x + player_2.x) / 2, (Player1.y + player_2.y) / 2)
    if (Player1.tileKindAt(TileDirection.Bottom, sprites.dungeon.collectibleInsignia) && player_2.tileKindAt(TileDirection.Bottom, sprites.dungeon.collectibleInsignia)) {
        game.gameOver(true)
    }
    if (info.score() == 0) {
        game.gameOver(false)
